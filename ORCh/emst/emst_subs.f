...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real(kind(1.d0)),intent(in)::sumwtk(msubn)
      real(kind(1.d0)),intent(out):: b(nkmax-1,msubn)
...
c               being executed.   Default sdev_small=1.d-4.
c     omdtmin  -The control parameter which decides whether the rejection method
c               will be used or not. Default omdtmin=1.d-6.
c    fphthr
c    rphthr :  -fphthr and rphthr are two parameters used to decide the dphthr.
c               default value for fphthr is 0.3d0. default value for rphthr is 1.d-5.
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
          implicit none
	       real(kind(1.d0)), intent(in) :: cvars(6)
          integer, intent(out) :: icheck,Max_Ave
	       real(kind(1.d0)), intent(out) :: sdev_small, omdtmin,
     &         fphthr,rphthr

              real(kind(1.d0)) sdev_def,omdt_def, fph_def,rph_def
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
            ...
            if(cvars(3) <= 0.d0) then
               sdev_small=sdev_def
            else
               sdev_small=cvars(3)
            endif

            if(cvars(4) <= 1.d-7 .or. cvars(4) > 1.d-2) then
              omdtmin  =omdt_def
            else
              omdtmin  =cvars(4)
            endif

            if(cvars(5) <= 0.d0)then
              fphthr=fph_def
            else
              fphthr=cvars(5)
            endif

            if(cvars(6) <= 0.d0)then
              rphthr=rph_def
            else
              rphthr=cvars(6)
            endif
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       real(kind(1.d0)),intent(in):: w(nd),y(nd,ndimd)
       real(kind(1.d0)),intent(out):: cov(ndimd,ndimd),ymean(ndimd)
       integer:: j,idim, jdim
       real(kind(1.d0))::  ave, s
       real(kind(1.d0)):: dsum
c
      dsum = 0.d0
...
c
	...
	double precision edwt(nkmax-1,msubn), w(npd), wtnode(npd),
     $      sumwt(msubn)
...
c	Check if this path has been traversed.
            ...
            if( wtnode(lc(node+istart-1)).eq.0.d0 )then
...
c	Check if this path has been traversed.
                ...
                if( wtnode(rs(ihroot+istart-1)).eq.0.d0 )then
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        implicit none
	integer, intent(in) :: n
	real(kind(1.d0)), intent(in) :: z0l,z0u,z1l,z1u
	real(kind(1.d0)), intent(inout) :: state(n)
        integer:: i, nt, int, notint
        real(kind(1.d0)):: p0,work(n)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        real(kind(1.d0)), intent(in) :: wt(np),  omdt, cvars(6),cons(4)
        real(kind(1.d0)), intent(inout) :: g(np,nc), state(np)

c       local variables

        integer:: i,j,imod,msubn,icheck,Max_Ave

        real(kind(1.d0))::  gtemp(np,nc),dphthr(nc)
        real(kind(1.d0))::  omtleft, MaxAvevar,range ,range_final,omdtm
        real(kind(1.d0))::  a(1),gtmax,sdev_small,omdtmin,fphthr,rphthr
...
c  -----------------for large omdt using IEM and return.
         ...
         if (range_final < sdev_small/10.d0) then
     ...
     1                    state)
         ...
         omtleft =omdt
         do while ( omtleft > 0.d0 ) !===keep mixing until omtleft=0 =====

            call Max_Avevar(Max_Ave,np,nc,g,wt,MaxAvevar,range)

c--------------perform IEM for small variance ---------------

            if(MaxAvevar < 1.d-12 .or. range < 4*sdev_small) then
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      implicit none
	integer, intent(in) :: np,nc
	real(kind(1.d0)), intent(in) :: wt(np), omdt
	real(kind(1.d0)), intent(inout) :: g(np,nc)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     $   msub(msubn,2)
      double precision phit(npd,ndim), phith(npd,ndim), w(npd),
     $   b(nkmax-1,msubn),eta,
...
c     
            coefi(node) = 1.d0/(coefi(node)*eta + w(isp(nodea)))
          enddo
c     
c     Start at the root node.
c
...
c     
     ...
     $             ( 1.d0 - coefi(node)*eta*coefe(node) )
                if(node.ne.iroot)then
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	  ...
	  real(kind(1.d0)), intent(in) ::  wt(np), g(np,nc)
	  real(kind(1.d0)), intent(out)::  MaxAvevar,range

        integer :: i,j
        real(kind(1.d0)) :: gmin(nc),gmax(nc)
...
c     type declaration of parameters passed
      ...
      double precision phi0(np, ncomp), wt(np), dt,  dtm, gtmax,
...
c     local type declarations for parameters
      ...
      double precision dphthr(ncomp),  z0l, z0u, z1l, z1u, 
     ...
     $    mode, iens, istart, ntsub, it, iabs, nq
      ...
      double precision phi(np,ncomp), b(np-1,msubn), sumwtk(msubn),
...
c     assign particle weights to wnm and normalize such that sum=1.d0
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      implicit none
      integer n, mode ,j
      double precision  ave, adev, sdev, var, skew, curt
      double precision data(n),w(n)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      double precision phi(npcd, ndimd), w(npcd), state(np), work(np),
...
c     local declarations
     ...
     $   ntadd, npsub, nwarn
      ...
      double precision pmin, p, pnew, vadd
     ...
     $   covtmp)
      ...
      if ( p .lt. pmin) then
        ...
        do iens = 1, msubn
          ...
          notint = nk - ntsub
          do i = 1, notint
            work(i) = dabs(state(isp(istart + ntsub - 1 + i)))
          enddo
          if ( notint .gt. 1 ) then
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	...
	real(kind(1.d0)), intent(out) :: x(n)

      integer i

      double precision f, pa, b2, a2
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     $   mst(npcd-1,2,msubn), isp(npcd)
      double precision phi(npcd,ncomp), varfn,  alpest,
     $   b(npcd-1,msubn)

c     Local declarations

      integer idim, iens, iedge, jabs, jnuabs, nk, istart
      double precision dvfndt, rbb, dphinu
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     $   rs(np), parent(np,2), stack(np), itrav(np)
      ...
      double precision phit(np, ncomp), phitmp(np, ncomp),
     $   phivfn(np, ncomp),
...
c     local declarations
     ...
     $	  ibrack, mxsrch, mbrack
      ...
      double precision ave, adev, sdev, svar, skew, curt, fh,
     $    eta2,
     $   etal, fl,  etflsp, etainc, varfnd, dvarfn, f,
     $   vfnprv, arg, del, fh0, fhl,  ainc
      double precision aa(1), dtmtemp
...
c     3.n Initialize root value etc
...
c  avoid fl~fh situation
          if(fl.gt.0.4d0*fh) then
            etal=1.10d0*etal
           else
            fhl  = fl / fh
            etal = ainc * etal / (1.0d0-fhl)
           endif
          isrch = isrch + 1
          goto 101
...
c     Accumulate dphi0
     ...
     $             dabs( phitmp(iabs,idim) - phit(iabs,idim))
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      implicit none
	integer, intent(in) :: n
	real(kind(1.d0)), intent(in) :: ds,z0l,z0u,z1l,z1u
	real(kind(1.d0)), intent(inout) :: state(n)

      integer :: i
      real(kind(1.d0)) :: work(n), p0, temp
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      double precision f(npd, ndim), avg(ndim), w(npd)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      double precision randn(n),w(n),wtmin
