...
c         < 0 for error in input
      ...
      real(kind(1.d0)), intent(in) :: wt(np), fscale(nc), omdt, cvars(6)
	   real(kind(1.d0)), intent(inout) :: f(np,nc), state(np)
...
c       local variables
        integer i,icheck
        double precision g(np,nc),cons(4)
...
c-------if icheck =1, check the inputs of wt,state,and fscale.
     ...
     &         minval(state(1:np)) < -cons(2) ) then
           ...
           if( minval(fscale(1:nc)) <= 0.d0 ) then
