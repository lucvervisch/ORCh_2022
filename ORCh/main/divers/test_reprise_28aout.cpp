...
int last_test = 0; //determine how many final tests will occur 
   ...
   for (int nbSpeciesToKeep=nsp_init-1; nbSpeciesToKeep>9; nbSpeciesToKeep--)
   {
      ...
      //Ecriture du schema V2 correspondant
      ...
      for (int k=0; k<nbSpeciesToKeepV2+1; k++)
         Species_to_addV2[int(sort_R_AD[nsp_init-1-k][1])] = true;
      Species_to_addV2[int(sort_R_AD[nsp_init-1-nbSpeciesToKeepV2][1])] = true;

      Write *w = new Write();
      w->Write_xml_file(initial_mech, mech_desc, outputSchemeNameV2, Species_to_addV2, Reactions_to_add, false, 
                        vector<double> (nreac_init, 0.0), vector<double> (nreac_init, 0.0), vector<double> (nreac_init, 0.0), vector<Species*> (), vector<Reaction*> ());
   ...
   {
      cout << "DRGEP Species step terminated. For next step, use the scheme \"" << outputSchemeName  <<"\" ." << endl; 
      break;
   }
   else if (last_test = 1 && fitness_final >= Te)
   {
      cout << "DRGEP Species step terminated. For next step, use the penultimate scheme tested." << endl; 
      break;
   }
